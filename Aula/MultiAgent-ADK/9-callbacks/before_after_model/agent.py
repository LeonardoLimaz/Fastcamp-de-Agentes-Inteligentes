...
def before_model_callback(
    callback_context: CallbackContext, llm_request: LlmRequest # Precisa-se também fornecer a requisição do LLM, diferente do callback do agente
    ...
    state = callback_context.state # Passa-se o estado da sessão
    agent_name = callback_context.agent_name # Pega o nome do agente

    # Extract the last user message
    last_user_message = "" # Extrai qual foi a última mensagem do usuário
    ...
    if last_user_message and "sucks" in last_user_message.lower(): # Verifica se a última mensagem do usuário possui a palavra inapropriada
        ...
        return LlmResponse( # Caso a mensagem tiver a palavra inapropriada, envia uma mensagem dizendo para refazer a frase em a palavra
    ...
    return None # Se não incluir a palavra inapropriada retorna None


def after_model_callback(
    callback_context: CallbackContext, llm_response: LlmResponse # Precisa-se também fornecer a resposta do LLM, diferente do callback do agente
) -> Optional[LlmResponse]:
    """
    ...
    if not llm_response or not llm_response.content or not llm_response.content.parts: # Caso o LLM não respondeu ou a resposta não tiver conteúdo retorna None
        return None

    # Extract text from the response
    response_text = "" # Extrai a resposta
    for part in llm_response.content.parts:
        if hasattr(part, "text") and part.text:
            response_text += part.text

    if not response_text: # Caso a reposta não tenha texto
        return None

    # Simple word replacements
    replacements = { # Dicionário de palabras a serem trocadas
        "problem": "challenge", # problem troca por challenge
        "difficult": "complex", # difficult troca por complex
    }

    # Perform replacements
    modified_text = response_text # Captura o texto de resposta do LLM
    modified = False

    for original, replacement in replacements.items(): # Troca as palavras que precisam ser trocadas
        if original in modified_text.lower():
            modified_text = modified_text.replace(original, replacement)
            modified_text = modified_text.replace(
                original.capitalize(), replacement.capitalize()
            )
            modified = True # Caso tenha sido feita trocas de palavras, define como True

    # Return modified response if changes were made
    if modified: # Se tiver sido defenido como True
        ...
        return LlmResponse(content=types.Content(role="model", parts=modified_parts)) # Retorna a resposta modificada do LLM 

    # Return None to use the original response
    return None # Caso não tenha nenhuma das palavras que precisam ser modificadas, continua tudo igual
