...
def get_capital_city(country: str) -> Dict[str, str]: # Recebe um país e verifica se a capital desse país está no dicionário da função
    """
...
def before_tool_callback(
    tool: BaseTool, args: Dict[str, Any], tool_context: ToolContext # Passa-se a ferramenta, os argumentos que são passados para a ferramenta e o contexto da ferramenta
) -> Optional[Dict]:
    """
...
def after_tool_callback(
    tool: BaseTool, args: Dict[str, Any], tool_context: ToolContext, tool_response: Dict # Passa-se a ferramenta, os argumentos que são passados para a ferramenta e o contexto da ferramenta
) -> Optional[Dict]:
    """
    ...
    if tool_name == "get_capital_city" and "washington" in original_result.lower(): # Verifica se a capital "washington" está na resposta da ferramenta
        ...
        modified_response["result"] = (
            f"{original_result} (Note: This is the capital of the USA. 🇺🇸)" # Se tiver "washington" na reposta adiciona essa nota
        )
