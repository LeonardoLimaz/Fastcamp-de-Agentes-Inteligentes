...
post_reviewer = LlmAgent(
    ...
    ## POST TO REVIEW
    {current_post}
    """, # Instrução de como o agente deve agir
    description="Reviews post quality and provides feedback on what to improve or exits the loop if requirements are met", # Descrição do propósito do agente
    tools=[count_characters, exit_loop], # Lista de ferramentas disponíveis
    output_key="review_feedback", # Salva a resposta do agente
)
