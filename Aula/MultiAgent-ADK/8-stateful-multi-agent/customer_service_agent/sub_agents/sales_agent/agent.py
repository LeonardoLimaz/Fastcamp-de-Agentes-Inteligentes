...
def purchase_course(tool_context: ToolContext) -> dict: # Inicia passando o contexto da ferramenta para que a ferramenta seja atualizada com o estado
    """
    ...
    current_purchased_courses = tool_context.state.get("purchased_courses", []) # Dentro do estado verifica quais cursos esse usuário já comprou 

    # Check if user already owns the course
    course_ids = [ # Verifica se o id do curso que o usuário está procurando já está na lista de cursos já comprados pelo usuário
    ...
    # Atualiza a lista de cursos que você já havia comprado, adicionando o novo curso comprado
    ...
    tool_context.state["interaction_history"] = new_interaction_history # Atualiza o histórico de interações para conter as informações de que o usuário comprou esse curso
...
sales_agent = Agent(
    ...
    description="Sales agent for the AI Marketing Platform course", # Descrição do que o agente faz
    instruction="""
    ...
    """, # Instrução de como o agente deve agir e o que ele deve fazer
    tools=[purchase_course], # Lista de ferramentas disponíveis
)
