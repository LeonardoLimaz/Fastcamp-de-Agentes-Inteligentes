...
def refund_course(tool_context: ToolContext) -> dict:
    ...
    current_purchased_courses = tool_context.state.get("purchased_courses", []) # Obtém o estado atual

    # Check if user owns the course
    course_ids = [ # Verifica se o usuário tem o curso
    ...
    # Remove o curso da lista de cursos possuídos
    ...
    tool_context.state["purchased_courses"] = new_purchased_courses # Atualiza o estado

    # Get current interaction history
    current_interaction_history = tool_context.state.get("interaction_history", []) # Atualiza o histórico de interações
    ...
    tool_context.state["interaction_history"] = new_interaction_history # Atualiza o estado com o novo histórico de interações
...
order_agent = Agent(
    ...
    description="Order agent for viewing purchase history and processing refunds", # Descrição do o agente faz
    instruction="""
    ...
    """, # Instrução de como ele deve agir e do que ele está apto a fazer
    tools=[refund_course, get_current_time], # Lista das ferramentas disponíveis para o agente
)
